...
@router.get("/capex-fuel-market/{model}")
async def get_capex_market(model: str):
    if model.lower() in ["none", "bau"]:
        if not os.path.exists(c.CAPEX_MARKET_TEMPLATE):
            return {"error": "Template file is missing"}

        return FileResponse(
            c.CAPEX_MARKET_TEMPLATE,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            filename=c.CAPEX_MARKET_TEMPLATE
        )

    FULL_CAPEX_MODEL_PATH = f"{c.CAPEX_MARKET_MODEL}{model}.xlsx"

    if not os.path.exists(c.CAPEX_MARKET_TEMPLATE):
        return {"error": "Template file is missing"}

    if not os.path.exists(FULL_CAPEX_MODEL_PATH):
        shutil.copy(c.CAPEX_MARKET_TEMPLATE, FULL_CAPEX_MODEL_PATH)
    ...
    xls_template = pd.ExcelFile(c.CAPEX_MARKET_TEMPLATE, engine="openpyxl")
    ...
    e.apply_formulas(FULL_CAPEX_MODEL_PATH, c.FORMULAS_JSON_PATH, models_sorted, expected_sheets, [])
